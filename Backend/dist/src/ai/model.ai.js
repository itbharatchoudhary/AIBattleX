...
const createGeminiClient = (modelName) => new ChatGoogle({
    model: modelName,
    apiKey: config.GOOGLE_API_KEY,
});
const isGeminiModelError = (errorMessage) => /not found for API version|unsupported for generateContent|invalid_request_error/i.test(errorMessage);
export const models = {
...
const isModelAvailable = (modelKey) => {
...
const recordModelFailure = (modelKey) => {
    const health = modelHealth[modelKey];
    if (!health)
        return;
    health.failures++;
...
const recordModelSuccess = (modelKey) => {
    const health = modelHealth[modelKey];
    if (!health)
        return;
...
const invokeModelWithRetry = async (model, prompt, modelKey) => {
...
const invokeModelWithRetryInternal = async (model, prompt, modelKey) => {
    for (let attempt = 0; attempt <= RETRY_ATTEMPTS; attempt++) {
        try {
            const response = await Promise.race([
                model.invoke(prompt),
                new Promise((_, reject) => setTimeout(() => reject(new Error("TIMEOUT")), attempt === 0 ? 15000 : 30000))
            ]);
            ...
            if (modelKey === 'gemini' && isGeminiModelError(errorMessage)) {
                const currentIndex = GEMINI_MODELS.indexOf(model.model || GEMINI_MODELS[0]);
                const nextModelName = GEMINI_MODELS[currentIndex + 1];
                if (nextModelName) {
                    console.log(`Gemini model ${(model.model)} unsupported, switching to ${nextModelName}`);
