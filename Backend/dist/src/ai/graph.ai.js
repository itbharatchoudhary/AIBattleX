...
const getResponseText = (response) => {
    ...
    if (Array.isArray(response.generations)) {
        ...
        if (typeof first === "string")
            return first;
        if (Array.isArray(first) && typeof first[0]?.text === "string")
            return first[0].text;
...
const solutionNode = async (state) => {
    ...
    const buildSolutionPrompt = (problem, solutionNumber, modelKey) => {
...
Answer:`;
    ...
    const getWorkingModel = async (preferredModel, fallbackModels, solutionNumber) => {
...
const judgeNode = async (state) => {
    ...
    const cleanJsonResponse = (text) => {
        ...
        const sanitizeMultilineJsonStrings = (jsonString) => {
            let inString = false;
            let escaped = false;
            let result = "";
    ...
    const getWorkingJudgeModel = async (preferredModel, fallbackModels) => {
        ...
        for (const judgeKey of judgeKeys) {
            ...
            const judgeLLM = models[judgeKey];
            if (!judgeLLM) {
