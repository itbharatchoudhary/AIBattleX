...
export const getHistory = async (req: any, res: any) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to fetch history", error: (err as Error).message });
  }
};
...
export const saveHistory = async (req: any, res: any) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to save history", error: (err as Error).message });
  }
};
...
export const deleteHistory = async (req: any, res: any) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to delete history", error: (err as Error).message });
  }
};
...
export const clearHistory = async (req: any, res: any) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Failed to clear history", error: (err as Error).message });
  }
};
