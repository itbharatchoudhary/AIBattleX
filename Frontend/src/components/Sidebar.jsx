  ...
  return (
    ...
    >
      ...
      <div className="flex items-center h-14 px-3 border-b border-slate-200 dark:border-white/10 shrink-0">
        <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-slate-600 to-slate-700 flex items-center justify-center shrink-0 shadow-lg shadow-slate-500/30">
                      ...
                      <>
                        ...
                        <span className="text-slate-500 dark:text-slate-400">
                          {entry.result.judge.solution_1_score > entry.result.judge.solution_2_score
                            ? `${MODELS[entry.models?.modelA]?.icon} ${MODELS[entry.models?.modelA]?.name}`
                            : entry.result.judge.solution_2_score > entry.result.judge.solution_1_score
                            ? `${MODELS[entry.models?.modelB]?.icon} ${MODELS[entry.models?.modelB]?.name}`
                            : '🤝 Tie'}
                        </span>
                      </>
                    )}
      ...
      <div className="px-2 py-3 border-t border-slate-200 dark:border-white/10 shrink-0">
        <div
          ...
          `}
        >
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-slate-500 to-slate-600 flex items-center justify-center text-sm shrink-0">
            {user?.avatar || '👤'}
          </div>
          {!collapsed && (
